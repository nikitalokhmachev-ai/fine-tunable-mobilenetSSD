...
with detection_graph.as_default():
  with tf.compat.v1.Session(graph=detection_graph) as sess:
    while True:
        if im_flag==1:
            image_np = cv2.imread('D:/Projects/TFMobileNet/frames/1.jpg')
            im_flag = 2
        elif im_flag==2:
            image_np = cv2.imread('D:/Projects/TFMobileNet/frames/2.jpg')
            im_flag = 3
        else: 
            image_np = cv2.imread('D:/Projects/TFMobileNet/frames/3.jpg')
            im_flag = 1
        ...
        for i in range(min(max_boxes_to_draw, boxes.shape[1])):
            if scores is None or scores[0][i] > 0.7:
                ...
                print ("This box is gonna get used {}(-1):".format(im_flag), boxes[0][i], classes[0][i])
